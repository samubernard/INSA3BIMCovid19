...
figure(1); clf;
semilogy(confirmed.Date, confirmed.Value)
hold on % do not replace data on plot
semilogy(death.Date, death.Value)
semilogy(recovered.Date, recovered.Value)

% text(datetime('2020-01-29'),1e3,'https://data.humdata.org/dataset/novel-coronavirus-2019-ncov-cases')

% add gov measures
line(datetime({'2020-03-16','2020-03-16'}),[1e0,1e4],'Color','r', 'LineStyle', '-.');
line(datetime({'2020-03-17','2020-03-17'}),[1e0,1e4],'Color','b', 'LineStyle', '--');

legend('Confirmed', 'Deaths', 'Recovered','Schools closed','Containement','Location','northwest')
xlabel('Date')
ylabel('Number')
