...
%% condition initiales
u( (X(:)).^2 + (Y(:)).^2 < 1 ) = 0.8; % condition initiale constante par
% morceaux
...
%Affichage des points de la ville A et de la ville B sans supprimer
%l'affichage de 'image' grâce à 'hold' :
hold on
plot(0,0,'o');
t1 = text(0,0,"Ville A");
t1.Color = 'red';
...
while t < tfinal 
    ...
    % Transfert de valeur entre deux zones :
    u(X(:).^2 + Y(:).^2 < 1 ) = u( X(:).^2 + Y(:).^2 < 1 ) -0.01;
    u((X(:)-5).^2 + (Y(:)-4).^2 < 1 ) = u( (X(:)-5).^2 + (Y(:)-4).^2 < 1 ) +0.01;
